...
inline static const float dPoint2(const float x_p, const float y_p, const float x, const float y);
...
inline static const float point_yGrad(const float attr, const float spr2,
			       const float x_p, const float y_p,
			       const float x, const float y);
inline static const float dLine(const float a_l, const float b_l, const float c_l,
			 const float x, const float y);
inline static const float dLine2(const float a_l, const float b_l, const float c_l,
			  const float x, const float y);
inline static const float lineHeight(const float attr, const float spr2,
			      const float a_l, const float b_l, const float c_l,
			      const float x, const float y);
...
class Charge {
 ...
 protected:
  const char *type;
  bool is_standard, affects_goalbox, line; // if it's not a line, it's a segment
 ...
 public:
  Charge(const char *type, float attraction, float spread, bool affects_goalbox,
	 float x1, float y1);
  Charge(const char *type, float attraction, float spread, bool affects_goalbox,
...
class PotentialField {
 ...
 public:
  ...
  const unsigned int addPointCharge(const float attraction, const float spread,
				    const float x, const float y,
  ...
  const unsigned int addLineCharge(const float attraction, const float spread,
				   ...
				   const bool affects_goalbox = true);
  const unsigned int addSegmentCharge(const float attraction, const float spread,
				      const float x1, const float y1,
				      const float x2, const float y2,
				      const bool affects_goalbox = true);
  ...
  void movementVectorAt(Vector2D *vector, bool *x_eq, bool *y_eq, const float at_x, const float at_y) const;
  // chargesDescription
