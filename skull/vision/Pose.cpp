...
float Pose::calcBodyCenterHeight() {
  ...
  left_leg_matrix.rotateY(body_roll_angle); // rotate body roll angle about y axis
  left_leg_matrix.rotateX(body_tilt_angle); // rotate body tilt angle about x axis
  // translates along x axis and y axis offsets
  left_leg_matrix.translate(-REAR_LEG_TO_CENTER_BODY_X,
			    REAR_LEG_TO_CENTER_BODY_Y,0);
  left_leg_matrix.rotateX(left_rear_shoulder_angle); // rotate shoulder angle along x
  left_leg_matrix.rotateY(left_rear_hip_angle); // rotate hip angle along y axis
  // translate from shoulder top to elbow
  left_leg_matrix.translate(ELBOW_TO_ELBOW_X,-ELBOW_TO_ELBOW_Y,
			    -SHOULDER_TO_ELBOW_LENGTH);
  left_leg_matrix.rotateX(left_rear_knee_angle); // rotate knee angle along x axis
  ...
  right_leg_matrix.translate(REAR_LEG_TO_CENTER_BODY_X,REAR_LEG_TO_CENTER_BODY_Y,0);  
  right_leg_matrix.rotateX(right_rear_shoulder_angle);
  right_leg_matrix.rotateY(right_rear_hip_angle);
  right_leg_matrix.translate(-ELBOW_TO_ELBOW_X,-ELBOW_TO_ELBOW_Y,-SHOULDER_TO_ELBOW_LENGTH);
...
float Pose::get2dDist(point <int> coord1, point <int> coord2) {
  return sqrt(pow(((float)(coord1.x-coord2.x)),2)+pow(((float)(coord1.y-coord2.y)),2));
}
...
void Pose::printMatrix(PoseMatrix matrix) {
  ...
  printf("%g %g %g %g", RAD2DEG(matrix.get(0)), RAD2DEG(matrix.get(1)), RAD2DEG(matrix.get(2)), matrix.get(3));
  printf("%g %g %g %g", RAD2DEG(matrix.get(4)), RAD2DEG(matrix.get(5)), RAD2DEG(matrix.get(6)), matrix.get(7));
  printf("%g %g %g %g", RAD2DEG(matrix.get(8)), RAD2DEG(matrix.get(9)), RAD2DEG(matrix.get(10)), matrix.get(11));
