...
void VisualCorner::determineCornerShape() {
  ...
  else if(Utility::tValueInMiddleOfLine(t2, line2.length, MIN_EXTEND_DIST)) {
    cornerType = T;
    tBar = line2;
    tStem = line1;
  }
...
const shape VisualCorner::getLClassification() {
  ...
  // Points from left to right; will flip the sign of both if the vector should
  // point in the opposite direction (the aforementioned end/start issue)
  pair <int, int> line1Basis = 
    VisualLine::getLineComponents(line1);
  pair <int, int> line2Basis = 
    VisualLine::getLineComponents(line2);
  
  // Find the line endpoint that's farther from the corner (allows us to direct
  // our line segment away from the corner)
  ...
  double dotProduct = (line1Basis.first * line2Basis.first) + 
    (line1Basis.second * line2Basis.second);

  // v dot w = ||v|| ||w|| cos theta -> v dot w / (||v|| ||w||) = cos theta 
  // -> ...
  double theta = DEG_OVER_RAD * acos(dotProduct/(line1.length * line2.length));
  /*
  ...
  */
  ...
  const double cornerToDogLineLength = Utility::getLength(dogLocation, cornerLocation);
  const double tOnCornerToDogLine = 
    Utility::findLinePointDistanceFromStart(intersection,
                                            dogLocation,
                                            cornerLocation,
                                            cornerToDogLineLength);
  
  const double otherLineLength = Utility::getLength(line1End, line2End);

  const double tOnOtherLine =
    Utility::findLinePointDistanceFromStart(intersection,
                                            line1End,
                                            line2End,
                                            otherLineLength);
