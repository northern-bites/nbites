...
sudo mv apache-ant-1.8.1/ /usr/local/ant

echo "setting global path variables"
sudo chmod a+w /etc/bashrc
echo "export WEBOTS_HOME=/Applications/Webots" >> /etc/bashrc
echo 'export PS1='\[\033[01;32m\]\u@\h\[\033[01;34m\] \w\[\033[31m\]`git branch 2>/dev/null|cut -f2 -d\* -s`\[\033[01;34m\]$\[\033[00m\] '' >>/etc/basrc
echo "source /usr/local/git/share/git-completion.sh" >> /etc/bashrc
...
pushd zlib-1.2.5 && \
    ./configure && make test && sudo make install && popd || exit 1
...
sudo mkdir /usr/local/nao-1.6
tar --strip 1 -C /usr/local/nao-1.6 -xjf mac-nao-sdk-1.6.m1.tar.bz2

echo "downloading Boost"
test -e boost_1_43_0.tar.gz || curl -OLk http://downloads.sourceforge.net/project/boost/boost/1.43.0/boost_1_43_0.tar.gz
