...
PaintField::PaintField(QWidget *parent): 
		   QWidget(parent)
{
  // nothing needed to initialize field
}

  // Gives a suggested size for Widget
QSize PaintField::sizeHint() const
{
  return QSize(800, 600);
}

  // Gives a minimum suggested size
QSize PaintField::minimumSizeHint() const
{
  return QSize(100, 100);
}

  // Meant for future so that we can update the drawing
  // of the ball
...
void PaintField::paintBall(){
   QPainter painter(this);\

 // Painting Ball
    painter.setBrush(Qt::red);
    painter.setPen(Qt::red);
    painter.drawEllipse(CENTER_FIELD_X, 
			CENTER_FIELD_Y, 
			BALL_SIZE, 
			BALL_SIZE);
}
