...
//                retVec.push_back(Block{yBufferSub, blackAttributes, "yBufferSub", "nbcross", 0, 0});
        ...
        for (int i = 0; i < module.subtractedWhiteSpots.size(); ++i) {
            ...
            spotAttrs["x"] = json::Number(module.subtractedWhiteSpots[i].ix());
            spotAttrs["y"] = json::Number(module.subtractedWhiteSpots[i].iy());
            spotAttrs["innerDiam"] = json::Number(module.subtractedWhiteSpots[i].innerDiam);
            
            whitespots.push_back(spotAttrs);
        }
