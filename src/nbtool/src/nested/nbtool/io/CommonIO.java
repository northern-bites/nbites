...
public class CommonIO {
	...
	protected static Socket setupNetSocket(String addr, int port) throws IOException {
		...
		if (address.isUnresolved()) {
			Debug.logf(Debug.ERROR, "CommonIO.setupNetSocket(): ERROR: Could not resolve address: " + addr);
			throw new UnresolvedAddressException();
		}
		try {
			newsock.connect(new InetSocketAddress(addr, port), ToolSettings.SOCKET_TIMEOUT);
		} catch (SocketTimeoutException ste) {
			Debug.logf(Debug.ERROR, "CommonIO.setupNetSocket() could not connect to socket within timeout!");
			throw ste;
		}
			...
			synchronized(this) {
				...
				if (socket != null) {
					try {
						...
						Debug.log(Debug.INFO, "Exception generated by IOInstance.kill():");
						e.printStackTrace();
					}
				}
			}
