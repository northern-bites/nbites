...
public abstract class VisionView extends ViewParent implements IOFirstResponder {
	...
	protected final Block getRobotBlock() {
		return latestVisionLog == null ? null : latestVisionLog.find("robotImage");
	}

	protected final Block getRobotCandidatesBlock() {
		if (latestVisionLog != null) {
			for (Block r : latestVisionLog.blocks) {
				if (r.type.equals(SharedConstants.SexprType_DEFAULT()) &&
						r.whereFrom.equals("nbcross-robot-candidates"))
					return r;
			}
		}
		return null;
