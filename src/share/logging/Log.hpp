...
#define NBL_CREATE_INT_CONSTANT_METHOD(name)    \
    int name() const { return dict.at(std::string(#name)).asConstNumber().asInt(); }
...
//        NBL_CREATE_INT_CONSTANT_METHOD(LOCAL_HOST_TIMEOUT)
    ...
    extern const SharedConstants CONSTANTS;
...
#define LOG_VERSION (8)
    ...
    private:
        ...
        Log() : createdWhen(time(NULL)), logClass(CONSTANTS.LogClass_Null()) { }
        Log(const std::vector<Block>& initBlocks,
            ...
            blocks(initBlocks), toplevelDictionary(initDict) {
                if (logClass == "") logClass = CONSTANTS.LogClass_Null();
            }
