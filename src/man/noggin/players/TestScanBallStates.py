...
def ballTracking(player):
    ...
    return player.stay()
#    if player.brain.tracker.target.framesOff <= constants.TRACKER_FRAMES_OFF_REFIND_THRESH:
...
#    if not player.brain.tracker.target.on:
#        if player.brain.tracker.target.framesOff > constants.TRACKER_FRAMES_OFF_REFIND_THRESH:
...
#            player.brain.tracker.target.framesOn >= constants.TRACKER_FRAMES_ON_TRACK_THRESH:
