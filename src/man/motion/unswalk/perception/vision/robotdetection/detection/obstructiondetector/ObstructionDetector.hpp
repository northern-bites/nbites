...
/**
 * Used to detect obstructions in the field edge.
 */
...
private:
    ...
    std::vector<int> getFieldEdges(VisionFrame &frame, const Fovea &saliency);


public:

	std::vector<Obstruction> complete(VisionFrame &frame, const Fovea &saliency);
};
