...
// http://runswift.cse.unsw.edu.au/confluence/download/attachments/3047440/100215-NaoForwardUNSWKinematicsLegToCamera.pptx.pdf?version=2&modificationDate=1266227985534 
...
static const float camera_bottom_angle = UNSWDEG2RAD(39.7);
static const float camera_top_angle = UNSWDEG2RAD(1.2);
...
void UNSWKinematics::updateDHChain() {
   ...
   coffsetY = UNSWDEG2RAD(parameters.cameraPitchTop);
   coffsetX = UNSWDEG2RAD(parameters.cameraYawTop);
   coffsetZ = UNSWDEG2RAD(parameters.cameraRollTop);
   ...
   coffsetY = UNSWDEG2RAD(parameters.cameraPitchBottom);
   coffsetX = UNSWDEG2RAD(parameters.cameraYawBottom);
   coffsetZ = UNSWDEG2RAD(parameters.cameraRollBottom);
...
UNSWKinematics::createCameraToFootTransform(Chain foot, bool top) {
   ...
   float bodyPitchOffset = UNSWDEG2RAD(parameters.bodyPitch);
...
void UNSWKinematics::setSensorValues(UNSWSensorValues sensorValues) {
