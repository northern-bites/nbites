...
struct UNSWRobotInfo
{
   ...
   UNSWRobotInfo (RRCoord rr, Type type, BBox imageCoords) :
         rr(rr),
         type(type),
   ...
   UNSWRobotInfo (RRCoord rr, Type type, BBox topImageCoords, BBox botImageCoords) :
       rr(rr),
       type(type),
       cameras(BOTH_CAMERAS),
   ...
   UNSWRobotInfo (RRCoord rr, Type type, BBox imageCoords, Cameras cameras) :
      rr(rr),
      type(type),
      cameras(cameras),
      topImageCoords(),
   ...
   bool operator== (const UNSWRobotInfo &other) const {
      return rr == other.rr;
   }
