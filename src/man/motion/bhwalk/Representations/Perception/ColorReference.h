...
class ColorReferenceBH : public Streamable
{
...
public:
  ...
  bool isNone(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isNone();
  }

  bool isOrange(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isOrange();
  }

  bool isYellow(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isYellow();
  }

  bool isBlue(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isBlue();
  }

  bool isWhite(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isWhite();
  }

  bool isGreen(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isGreen();
  }

  bool isBlack(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isBlack();
  }

  bool isRed(const ImageBH::Pixel* pixel) const
  {
    return getColorClasses(pixel).isRed();
  }

  ColorReferenceBH::MultiColor getColorClasses(const ImageBH::Pixel* pixel) const;
...
private:
  ...
  ColorReferenceBH::MultiColor getColorClassesFromHSI(const ImageBH::Pixel& pixel) const;
  void update();
