...
class ThumbnailBH : public Streamable
{
public:
...
template<class Pixel>
ThumbnailBH::TImage<Pixel>::TImage() : width(maxWidth), height(maxHeight)
{
...
template<class Pixel>
ThumbnailBH::TImage<Pixel>::TImage(const TImage& other) : width(other.maxWidth), height(other.maxHeight)
{
  imagePadding = static_cast<Pixel*>(SystemCall::alignedMalloc(maxWidth * maxHeight * sizeof(Pixel) + (padding * 2), 16));
  image = imagePadding + (padding / sizeof(Pixel));
...
template<class Pixel>
ThumbnailBH::TImage<Pixel>::~TImage()
{
  SystemCall::alignedFree(imagePadding);
}

template<class Pixel>
void ThumbnailBH::TImage<Pixel>::setResolution(int width, int height)
{
...
template<class Pixel>
ThumbnailBH::TImage<Pixel>& ThumbnailBH::TImage<Pixel>::operator=(const TImage& other)
{
  width = other.width;
  height = other.height;
  memcpy(image, other.image, maxWidth * maxHeight * sizeof(Pixel));
...
template<class Pixel>
void ThumbnailBH::TImage<Pixel>::serialize(In* in, Out* out)
{
...
template<class Pixel, class PixelUncompressed>
void ThumbnailBH::TImageCompressed<Pixel, PixelUncompressed>::compress(const TImage<PixelUncompressed>& uncompressedImage)
{
...
template<class Pixel, class PixelUncompressed>
void ThumbnailBH::TImageCompressed<Pixel, PixelUncompressed>::uncompress(TImage<PixelUncompressed>& uncompressedImage) const
{
