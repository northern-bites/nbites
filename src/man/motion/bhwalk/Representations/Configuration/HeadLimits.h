...
STREAMABLE(HeadLimitsBH,
{
private:
  bool intersectionWithShoulderPlane(const RobotCameraMatrixBH& robotCameraMatrix,
                                     const Vector3BH<>& shoulderInOrigin,
                                     const float imageTilt, Vector3BH<>& intersection) const;

public:
  Vector2BH<> getTiltBound(float pan) const;
   ...
   */
  bool imageCenterHiddenByShoulder(const RobotCameraMatrixBH& robotCameraMatrix,
                                   const Vector3BH<>& shoulderInOrigin, const float imageTilt,
   ...
   */
  bool intersectionWithShoulderEdge(const RobotCameraMatrixBH& robotCameraMatrix,
                                    const Vector3BH<>& shoulderInOrigin, Vector3BH<>& intersection) const;
