...
void SpecialActions::MotionNetData::load(In& stream)
{
  ...
  for(int i = 0; i < numberOfNodes; ++i)
  {
    ...
    case 2:
      ...
      stream >> nodeArray[i].d[1] >> nodeArray[i].d[JointDataBH::numOfJoints + 3];
      break;
    case 1:
      nodeArray[i].d[0] = (short) MotionNetNode::typeConditionalTransition;
      stream >> nodeArray[i].d[1] >> nodeArray[i].d[2] >> nodeArray[i].d[JointDataBH::numOfJoints + 3];
      break;
    case 4:
      nodeArray[i].d[0] = (short) MotionNetNode::typeHardness;
      for(int j = 1; j < JointDataBH::numOfJoints + 3; j++)
        stream >> nodeArray[i].d[j];
      break;
    case 3:
      nodeArray[i].d[0] = (short) MotionNetNode::typeData;
      for(int j = 1; j < JointDataBH::numOfJoints + 1; ++j)
      {
        stream >> nodeArray[i].d[j];
        if(nodeArray[i].d[j] != JointDataBH::off    &&
           nodeArray[i].d[j] != JointDataBH::ignore)
          nodeArray[i].d[j] = fromDegrees(nodeArray[i].d[j]);
      }
      for(int k = JointDataBH::numOfJoints + 1; k < JointDataBH::numOfJoints + 4; ++k)
    ...
    for(std::vector<SpecialActionInfo>::const_iterator it = infos.specialActionInfos.begin(); it != infos.specialActionInfos.end(); it++)
    {
      ...
      if(it->type == SpecialActionInfo::once || it->type == SpecialActionInfo::homogeneous)
      {
        ...
        if(it->type == SpecialActionInfo::homogeneous)
        {
          ...
          float motionCycleTime = theFrameInfoBH.cycleTime;
...
bool SpecialActions::getNextData(const SpecialActionRequest& specialActionRequest,
                                 SpecialActionsOutputBH& specialActionsOutput)
{
...
void SpecialActions::calculateJointRequest(JointRequestBH& jointRequest)
{
  float ratio, f, t;
  ...
  if(interpolationMode)
  {
    ...
    for(int i = 0; i < JointDataBH::numOfJoints; ++i)
    {
      ...
      if(!mirror)
        ...
        t = currentRequest.mirror((JointDataBH::Joint)i);
      // if fromAngle is off or ignore use JointDataBH for further calculation
  ...
  else
  {
    ...
    for(int i = 0; i < JointDataBH::numOfJoints; i++)
    {
      ...
      if(!mirror)
        ...
        t = currentHardnessRequest.mirror((JointDataBH::Joint)i);
      if(t == f)
      ...
      else
      {
        ...
        if(t == HardnessData::useDefault)
          t = mirror ? theHardnessSettingsBH.mirror((JointDataBH::Joint)i) : theHardnessSettings.hardness[i];
...
void SpecialActions::update(SpecialActionsOutputBH& specialActionsOutput)
{
  float speedFactor = 1.0f;
  MODIFY("parameters:SpecialActions:speedFactor", speedFactor);
  if(theMotionSelectionBH.specialActionMode != MotionSelectionBH::deactive)
  {
    ...
    if(dataRepetitionCounter <= 0)
    {
      if(!wasActive)
      {
        ...
        currentNode = 0;
        for(int i = 0; i < JointDataBH::numOfJoints; ++i)
          lastRequest.angles[i] = theFilteredJointDataBH.angles[i];
        lastSpecialAction = SpecialActionRequest::numOfSpecialActionIDs;
      }
      ...
      if(!getNextData(theMotionSelectionBH.specialActionRequest, specialActionsOutput))
      {
        wasActive = true;
        wasEndOfSpecialAction = true;
        specialActionsOutput.odometryOffset = Pose2DBH();
        return;
      }
    }
    else
    {
      dataRepetitionCounter -= int(theFrameInfoBH.cycleTime * 1000 * speedFactor);
      hardnessInterpolationCounter -= int(theFrameInfoBH.cycleTime * 1000 * speedFactor);
    }
    ...
    if(currentInfo.type == SpecialActionInfo::homogeneous || currentInfo.type == SpecialActionInfo::once)
      if(mirror)
        specialActionsOutput.odometryOffset = Pose2DBH(-currentInfo.odometryOffset.rotation, currentInfo.odometryOffset.translation.x, -currentInfo.odometryOffset.translation.y);
      else
    ...
    if(deShakeMode)
      for(int i = JointDataBH::LShoulderPitch; i <= JointDataBH::RElbowRoll; ++i)
        if(randomFloat() < 0.25)
          specialActionsOutput.angles[i] = JointDataBH::off;
  }
  wasActive = theMotionSelectionBH.specialActionMode == MotionSelectionBH::active;
}
