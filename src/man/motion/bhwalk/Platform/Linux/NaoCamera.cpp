  ...
  for(int i = 0; i < 2; ++i)
  {
    if(pollfds[i].revents & POLLIN)
    {
      ...
      else
      {
        ...
        {
          cams[i]->first = false;
          printf("%s camera is working\n", CameraInfoBH::getName(cams[i]->camera));
        }
      }
    }
...
void NaoCamera::setSettings(const CameraSettingsBH& newset)
{
  if(settings == newset)
    return;
...
unsigned int NaoCamera::writeCameraSettings()
{
  ...
  std::list<CameraSettingsBH::V4L2Setting> v4l2settings = settings.getSettings();
  std::list<CameraSettingsBH::V4L2Setting> appliedv4l2settings = appliedSettings.getSettings();
