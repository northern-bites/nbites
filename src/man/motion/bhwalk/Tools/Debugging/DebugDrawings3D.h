...
#define DRAWDIGIT3D(id, digit, pos, digitsize, linewidth, color) do\
  {\
    ...
    for(int i = 0; i < 7; ++i)\
      if(digit & (1 << i))\
      {\
        Vector3BH<> from = pos - points[i] * digitsize;\
        Vector3BH<> to = pos - points[i + 1] * digitsize;\
...
#define QUAD3D(id, corner1, corner2, corner3, corner4, color) \
  ...
  { \
    OUTPUT(idDebugDrawing3D, bin, \
           ...
           (char)Global::getDrawingManager3D().getDrawingId(id) << \
           Vector3BH<>(corner1) << Vector3BH<>(corner2) << Vector3BH<>(corner3) << Vector3BH<>(corner4) <<\
           ColorRGBA(color) \
          ); \
  })
...
#define CUBE3D(id, a, b, c, d, e, f, g, h, size, color) \
  ...
  { \
    OUTPUT(idDebugDrawing3D, bin, \
           ...
           (char)Global::getDrawingManager3D().getDrawingId(id) << \
           Vector3BH<>(a) << Vector3BH<>(b) << Vector3BH<>(c) << Vector3BH<>(d) << Vector3BH<>(e) << Vector3BH<>(f) << \
           Vector3BH<>(g) << Vector3BH<>(h) <<\
           (float)(size) << \
           ColorRGBA(color) \
          ); \
  })
...
#define CYLINDERARROW3D(id, from, to, radius, arrowlen, arrowradius, color) do \
  { \
    Vector3BH<> forward = to - from; \
    float height = forward.abs() - arrowlen; \
    forward.normalizeBH(height * 0.5f); \
    Vector3BH<> center = from + forward; \
    float rx = 0.f, ry = 0.f; \
    ...
    forward.normalizeBH(arrowlen * 0.5f); \
    Vector3BH<> arrowCenter = to - forward; \
