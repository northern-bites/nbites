...
* @param id A drawing id
* @param center The coordinate of the center of the ellipse (Vector2BH<>)
...
* @param id A drawing id
* @param topLeft The coordinate of the top left corner of the rectangle without rotation (Vector2BH<>)
* @param rotation The rotation of the x axis
...
#define POSE_2D_SAMPLE(id, p, color) \
  ...
  { \
    ...
    current = p; current += Pose2DBH(-40,-40); \
    LINE(id, int(current.translation.x), int(current.translation.y), int(p.translation.x), int(p.translation.y), \
         1, Drawings::ps_solid, color); \
    current = p; current += Pose2DBH(-40,40); \
    LINE(id, int(current.translation.x),int(current.translation.y),int(p.translation.x), int(p.translation.y), \
...
#define FILLED_RECTANGLE(id, x1,y1,x2,y2, penWidth, penStyle, penColor, fillStyle, fillColor) \
  COMPLEX_DRAWING(id, \
  { \
    Vector2BH<int> points[4]; \
    points[0] = Vector2BH<int>(x1, y1); \
    points[1] = Vector2BH<int>(x2, y1); \
    points[2] = Vector2BH<int>(x2, y2); \
    points[3] = Vector2BH<int>(x1, y2); \
...
/**
* A macro that sends a RobotPoseBH (Pose2DBH)
* @param id A drawing id
* @param p The desired Pose2DBH
* @param color The desired color for this drawing
*/
#define DRAW_ROBOT_POSE(id, p, color) \
  COMPLEX_DRAWING(id, \
  { \
    Vector2BH<> bodyPoints[4] = {Vector2BH<>(55, 90), Vector2BH<>(-55, 90), \
                               Vector2BH<>(-55, -90), Vector2BH<>(55, -90)}; \
    Vector2BH<> translation = p.translation; \
    float rotation = p.rotation; \
    for(int i = 0; i < 4; i++) \
      bodyPoints[i] = p * bodyPoints[i]; \
    Vector2BH<> dirVec(200.f, 0.f); \
 ...
 * @param id A drawing id
 * @param cov The covariance matrix as Matrix2x2BH<>
