...
public:
   ...
   */
  SpatialTransform(const RotationMatrixBH& rot, const Vector3BH<>& pos, bool posInverted) : rot(rot)
  {
  ...
  {
    rot *= other.rot;
    // If rot is B in A coordinates, pos is A in B coordinates. That's why it doesn't look like Pose3DBH!
  ...
  {
    const RotationMatrixBH rotT = rot.invert();
    const Vector3BH<> rotTc = rotT * inertia.centerOfMass;
    const Matrix3x3BH<> posCross = SpatialVector<>::crossProductMatrix(pos);
    const Matrix3x3BH<> rotTcCross = SpatialVector<>::crossProductMatrix(rotTc);
    const Vector3BH<> comNew = rotTc + pos;
    const Matrix3x3BH<> comNewCross = SpatialVector<>::crossProductMatrix(comNew);
  ...
  SpatialInertiaDerivative transform(const SpatialInertiaDerivative& inertiaDerivative) const
  {
    ...
    Matrix3x3BH<> topRight = rotT * inertiaDerivative.topRight * rot;
    // Compute top left block of transformed inertia derivative
    Matrix3x3BH<> rx = SpatialVector<>::crossProductMatrix(pos);
    Matrix3x3BH<> topLeft = rotT * inertiaDerivative.topLeft * rot;
    topLeft -= topRight * rx;
    topLeft -= rx * topRight;
  ...
  /**
   * Create a Pose3DBH corresponding to this spatial transform.
   */
  Pose3DBH toPose3D() const
  {
    const Vector3BH<> translation = rot * (-pos);
    return Pose3DBH(rot, translation);
  }
