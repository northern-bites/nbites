...
public:
  ...
  SpatialInertiaDerivative(const Matrix3x3BH<>& topLeft, const Matrix3x3BH<>& topRight)
  ...
  SpatialVector<> operator*(const SpatialVector<>& vector) const
  {
    ...
    Matrix3x3BH<> bottomLeft = topRight * (-1.0f);
  ...
  SpatialInertiaDerivative derive(const SpatialVector<>& vector) const
  {
    ...
    const Matrix3x3BH<> dTopLeft = wx * topLeft - vx * topRight - topLeft * wx - topRight * vx;
    const Matrix3x3BH<> dTopRight = wx * topRight - topRight * wx;
