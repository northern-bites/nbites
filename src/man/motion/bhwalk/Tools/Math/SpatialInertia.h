...
public:
  ...
  SpatialInertia(const Matrix3x3BH<>& momentOfInertia, const Vector3BH<>& centerOfMass, float mass)
  ...
  SpatialVector<> operator*(const SpatialVector<>& vector) const
  {
    ...
    Vector3BH<> massMoment = centerOfMass * mass;
  ...
  SpatialVector<> inverseApply(const SpatialVector<>& vector) const
  {
    ...
    Matrix3x3BH<> cx = SpatialVector<>::crossProductMatrix(centerOfMass);
    Matrix3x3BH<> cxT = cx.transpose(); // To consider: Replace with -cx?
    Matrix3x3BH<> moiComInv = momentOfInertia - cx * cxT * mass;
    moiComInv = moiComInv.invert();

    // n -> vector.top, f -> vector.bottom. To consider: Replace multiplications with cx with cross products?
    Vector3BH<> ncxTf = vector.top + cxT * vector.bottom;
  ...
  SpatialInertiaDerivative derive(const SpatialVector<>& vector) const
  {
    ...
    const Vector3BH<> wxc =  vector.top ^ centerOfMass;
    Matrix3x3BH<> topRight = SpatialVector<>::crossProductMatrix(wxc + vector.bottom);
    topRight *= mass;

    // Calculate top left block of SpatialInertiaDerivative
    const Matrix3x3BH<> wx = SpatialVector<>::crossProductMatrix(vector.top);
    const Matrix3x3BH<> vx = SpatialVector<>::crossProductMatrix(vector.bottom);
    const Matrix3x3BH<> cx = SpatialVector<>::crossProductMatrix(centerOfMass);
    const Matrix3x3BH<> topLeft = wx * momentOfInertia - momentOfInertia * wx - (vx * cx + cx * vx) * mass;
  ...
  Matrix3x3BH<> momentOfInertia;
  Vector3BH<> centerOfMass;
  float mass;
