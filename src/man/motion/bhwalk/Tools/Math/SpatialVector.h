...
public:
  ...
  SpatialVector<V>(const Vector3BH<V>& top, const Vector3BH<V>& bottom, bool motion = true) : top(top), bottom(bottom), motion(motion) {}
  ...
  Vector3BH<> top;    /*< The top block of this vector */
  Vector3BH<> bottom; /*< The bottom block of this vector */
  bool motion;      /*< True if this is a spatial motion vector. */

  /**
   * Computes a matrix V of a vector v such that Vp = v x p.
   */
  static Matrix3x3BH<> crossProductMatrix(const Vector3BH<>& v)
  {
    return Matrix3x3BH<>(0.0f, -v.z, v.y,
                       v.z,  0.0f, -v.x,
                       -v.y, v.x,  0.0f);
  }
};
