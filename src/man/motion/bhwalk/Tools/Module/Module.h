...
*   REQUIRES(ImageBH)                                      // Has to be updated before
*   REQUIRES(CameraMatrixBH)                               // Has to be updated before
*   USES(RobotPoseBH)                                      // Is used, but has not to be updated before
*   PROVIDES_WITH_MODIFY(BallPerceptBH)                    // Class provides a method to update BallPerceptBH. Representation can be MODIFYed.
...
*   DEFINES_PARAMETER(CameraInfoBH, Camera, camera, lower) // Use an enum as parameter that was defined in a certain class.
* END_MODULE
*
...
* {
*   void update(BallPerceptBH& ballPercept);
...
*/
template<class M, class B> class ModuleBH : public ModuleBase
{
private:
  /**
...
public:
  ...
  * @param category The name of the category of this module.
  */
  ModuleBH(const char* name, const char* category)
...
#define REQUIRES(representation) \
  public: using Blackboard::the##representation; \
  \
  /** \
...
* @param category The name of the category of this module.
*/
#define MAKE_SOLUTION(module, base, category) ModuleBH<module, base##Base> the##module##ModuleBH(#module, #category);
