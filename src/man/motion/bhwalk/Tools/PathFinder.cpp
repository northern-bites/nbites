...
void PathFinder::findPath(const Vector2BH<>& startOfPath, const Vector2BH<>& endOfPath)
{
  ...
  Vector2BH<> oldStart = startOfPath;
  Vector2BH<> oldEnd = endOfPath;

  Node startPosition(startOfPath);
  Node endPosition(endOfPath);

  Vector2BH<> positionOfObstacleStart;
  Vector2BH<> positionOfObstacleEnd;
  float smallestDistanceStart;
  float smallestDistanceEnd;

  vector<Node> allObstacles; // puts all obstacles into one vector
  for(std::vector<GaussianPositionDistribution>::const_iterator i = theCombinedWorldModelBH.positionsOpponentTeam.begin(), end = theCombinedWorldModelBH.positionsOpponentTeam.end(); i != end; ++i)
  {
    allObstacles.push_back(Node(i->robotPosition));
  }
  for(std::vector<Pose2DBH>::const_iterator i = theCombinedWorldModelBH.positionsOwnTeam.begin(), end = theCombinedWorldModelBH.positionsOwnTeam.end(); i != end; ++i)
  {
    if((i->translation - startOfPath).squareAbs() > sqrBH(100) && (i->translation - endOfPath).squareAbs() > sqrBH(100)) // own robot is not added to all obstacles
    {
  ...
  if(smallDistance || inSameObstacle || inOverlappingObstacles) // if start and end position can still be connected
  {
    ...
    collides = checkForCollision(allObstacles, startPosition.position, endPosition.position, true, distanceToObstacle, positionObstacle, endPosition.position);
#endif
    Vector2BH<> directionObstacle = positionObstacle - startPosition.position;
    Vector2BH<> directionPositions = endPosition.position - startPosition.position;
  ...
  else // if necessary use positions outside the obstacle
  {
    ...
    {
      Vector2BH<> positionOfObstacle = i == 0 ? positionOfObstacleStart : positionOfObstacleEnd;
  ...
  else if(countNoPathFound < parameters.counterUseOldPath && completePath.size() != 0) // use old path if no new path was found
  {
    ...
    for(int i = 0; i <= size; i++)
    {
      ...
      Vector2BH<> rotatedDirection = line.direction;
      rotatedDirection.rotateLeft();

      Vector2BH<> positionRelToBase = startOfPath - line.base;
...
void PathFinder::createRandomPosition(Vector2BH<>& randomPosition, const vector<Node>& currentNotUsedTree)
{
  ...
  {
    randomPosition.x = randomFloat() * ((theFieldDimensionsBH.xPosOpponentFieldBorder * parameters.searchSpaceFactor) - (theFieldDimensionsBH.xPosOwnFieldBorder * parameters.searchSpaceFactor)) + (theFieldDimensionsBH.xPosOwnFieldBorder * parameters.searchSpaceFactor); // finds a random position on field, random points outside the field are used, so that the whole field can be expanded. PathBH is clipped on field later
    randomPosition.y = randomFloat() * ((theFieldDimensionsBH.yPosLeftFieldBorder * parameters.searchSpaceFactor) - (theFieldDimensionsBH.yPosRightFieldBorder * parameters.searchSpaceFactor)) + (theFieldDimensionsBH.yPosRightFieldBorder * parameters.searchSpaceFactor);
  }
}

void PathFinder::calculateNearestNode(const vector<Node>& currentUsedTree, int& indexNearestNode, const Vector2BH<>& randomPosition)
{
...
void PathFinder::calculateNearestNodeMMX(const vector<Node>& currentUsedTree, int& indexNearestNode, const Vector2BH<>& randomPosition)
{
#ifdef TARGET_ROBOT
  _m_femms();
...
bool PathFinder::checkForCollision(const vector<Node>& allObstacles, const Vector2BH<>& nearestNode, const Vector2BH<>& randomPosition, bool usePosition, float& distanceToObstacle, Vector2BH<>& positionObstacle, const Vector2BH<>& target)
{
  bool collides = false;
  float currentDistance = parameters.distanceToObstacle;
  Vector2BH<> newPosition;
  ...
  {
    newPosition = nearestNode + (randomPosition - nearestNode).normalizeBH(parameters.stepSize); // calculate endPosition
  }
  if(std::abs(newPosition.x) > (theFieldDimensionsBH.xPosOpponentGroundline + (theFieldDimensionsBH.xPosOpponentFieldBorder - theFieldDimensionsBH.xPosOpponentGroundline) / 2) || std::abs(newPosition.y) > (theFieldDimensionsBH.yPosLeftSideline + (theFieldDimensionsBH.yPosLeftFieldBorder - theFieldDimensionsBH.yPosLeftSideline) / 2)) // if new node is outside the field
  {
    collides = true;
    return collides;
  }
  // path should be around the own penalty area for avoiding illegal defenders (Only for field players):
  if(newPosition.x < theFieldDimensionsBH.xPosOwnPenaltyArea + 200 && std::abs(newPosition.y) < theFieldDimensionsBH.yPosLeftPenaltyArea + 200 && theRobotInfoBH.number != 1)
  {
    if(!(theGameInfoBH.state == STATE_READY && parameters.enterPenaltyAreaInReady && newPosition.x > theFieldDimensionsBH.xPosOwnPenaltyArea - 200))
    {
      collides = true;
      return collides;
    }
  ...
  Vector2BH<> rotatedDirection = segmentOfPath.direction;
  rotatedDirection.rotateLeft();
  for(std::vector<Node>::const_iterator i = allObstacles.begin(), end = allObstacles.end(); i != end; ++i)
  {
    Vector2BH<> obstacleRelToBase = i->position - segmentOfPath.base;
...
bool PathFinder::checkForCollisionMMX(const vector<Node>& allObstacles, const Vector2BH<>& nearestNode, const Vector2BH<>& randomPosition, bool usePosition, float& distanceToObstacle, Vector2BH<>& positionObstacle, const Vector2BH<>& target)
{
  bool collides = false;
#ifdef TARGET_ROBOT
  float currentDistance = parameters.distanceToObstacle;
  ...
  if(std::abs(newPosition.x) > (theFieldDimensionsBH.xPosOpponentGroundline + (theFieldDimensionsBH.xPosOpponentFieldBorder - theFieldDimensionsBH.xPosOpponentGroundline) / 2) || std::abs(newPosition.y) > (theFieldDimensionsBH.yPosLeftGroundline + (theFieldDimensionsBH.yPosLeftFieldBorder - theFieldDimensionsBH.yPosLeftGroundline) / 2)) // if new node is outside the field
  {
    collides = true;
    return collides;
  }
  ...
  Vector2BH<> middlePosition = (nearestNode + newPosition) / 2; // calculates the middle of the segment
  Node middleNode = Node(middlePosition);
  float nearestDistance = sqrBH(((newPosition - nearestNode).abs() / 2) + parameters.distanceToObstacle);
  Vector2BH<> vectorNearestDistance = Vector2BH<>(nearestDistance, nearestDistance);
  ...
  for(int i = 0; i < vectorSize - 1; i += 2) // finds the node in the tree which is nearest to the random position, size - 1 for odd number of nodes
  {
    __m64 evenElem = *((__m64*) & (firstElem[i].position.x));
    __m64 oddElem = *((__m64*) & (firstElem[i + 1].position.x));
    //calculates the square distance between the element and the middle position (is the same as squareAbs() from Vector2BH<>)
  ...
  for(unsigned int i = 0; i < nearObstacles.size(); i++)
  {
    if(nearObstacles[i])
    {
      Vector2BH<> obstacleRelToBase = allObstacles[i].position - segmentOfPath.base;
      bool obstacleIsRelevant = (rotatedDirection.x * obstacleRelToBase.y - obstacleRelToBase.x * rotatedDirection.y) < 0; // if obstacle is between the positions. Obstacles behind the posititons (not between) are not relevant
...
void PathFinder::checkForFoundPath(const Vector2BH<>& currentUsedNode, const vector<Node>& currentNotUsedTree, bool& foundPath, bool& foundPathInFirstTree, int& indexOtherTree, const bool useFirstTree)
{
  float distanceBetweenTrees = sqrBH(parameters.distancePathCombination);  // squared for faster comparison
...
void PathFinder::checkForFoundPathMMX(const Vector2BH<>& currentUsedNode, const vector<Node>& currentNotUsedTree, bool& foundPath, bool& foundPathInFirstTree, int& indexOtherTree, const bool useFirstTree)
{
#ifdef TARGET_ROBOT
  _m_femms();
  ...
  if(vectorSize == 1)
  {
    float distanceBetweenTrees = sqrBH(parameters.distancePathCombination);
  ...
  if((currentNotUsedTree[indexNearestNode].position - currentUsedNode).squareAbs() <= sqrBH(parameters.distancePathCombination)) // check if distance of the nearest node is small enough
  {
...
void PathFinder::createNewNode(vector<Node>& currentUsedTree, const Vector2BH<> randomPosition, const int indexNearestNode)
{
  Vector2BH<> positionNewNode;
  ...
  {
    positionNewNode = currentUsedTree[indexNearestNode].position + (randomPosition - currentUsedTree[indexNearestNode].position).normalizeBH(parameters.stepSize);
  }
...
void PathFinder::createCompletePath(vector<Node>& completePath, const bool foundPathInFirstTree, const vector<Node>& firstTree, const vector<Node>& secondTree, const int indexOtherTree, const bool obstacleStart, const bool obstacleEnd, const Vector2BH<>& oldStart, const Vector2BH<>& oldEnd)
{
...
bool PathFinder::checkForObstaclesNearPosition(const vector<Node>& allObstacles, Vector2BH<>& positionOfObstacle, const Vector2BH<>& position, float& smallestDistance)
{
  float currentDistance = parameters.distanceToObstacle;
...
bool PathFinder::checkForObstaclesNearPositionMMX(const vector<Node>& allObstacles, Vector2BH<>& positionOfObstacle, const Vector2BH<>& position, float& smallestDistance)
{
  bool foundObstacle = false;
#ifdef TARGET_ROBOT
...
void PathFinder::addAvoidingPoints(const vector<Node>& allObstacles, vector<Node>& currentUsedTree, const Vector2BH<>& position, const Vector2BH<>& positionOfObstacle, bool nearestObstacleMMX, const Node& target, const bool startIsUsed)
{
  int degree = 60;
  Vector2BH<> originalPosition = position;
  float length;
  Vector2BH<> difference;
  if(position == positionOfObstacle)// special handling if obstacle = position
  {
    length = parameters.distanceToObstacle;
    difference = Vector2BH<>(parameters.distanceToObstacle, 0.0f);
  }
  else
  {
    difference = position - positionOfObstacle;
    difference.normalizeBH(parameters.distanceToObstacle - difference.abs());
    float hypotenuse = (position - positionOfObstacle).abs();
    float opposite = sin(fromDegrees(degree)) * hypotenuse;
    float adjacent = sqrt(sqrBH(hypotenuse) - sqrBH(opposite));
