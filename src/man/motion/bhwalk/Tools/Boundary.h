...
{
public:
  RangeBH<T> x, /**< The range in x-direction. */
  ...
  */
  Boundary(const RangeBH<T>& x, const RangeBH<T>& y) : x(x), y(y) {}

  /**
  * The function enlarges the boundary so that it also includes the specified point.
  * @param p The point.
  */
  void add(const Vector2BH<T>& p)
  {
    x.add(p.x);
    y.add(p.y);
  }
  ...
  */
  bool isInside(const Vector2BH<T>& p) const
