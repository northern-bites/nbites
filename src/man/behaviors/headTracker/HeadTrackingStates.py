...
def stopped(headTracker):
    return headTracker.stay()

def faceForward(headTracker):
    if headTracker.firstFrame():
        request = headTracker.brain.interface.motionRequest
        request.stop_head = True
        request.timestamp = int(headTracker.brain.time * 1000)

        headTracker.helper.executeHeadMove(headTracker.headMoves.ZERO_HEADS)

    if not headTracker.brain.motion.head_is_active:
        return headTracker.goLater('stopped')

    return headTracker.stay()

def tracking(headTracker):
    print "asdf"
    return headTracker.stay()

def snap(headTracker):
    # print "snap"
    return headTracker.stay()
