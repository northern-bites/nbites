...
/* This function takes in a blob generated by the spot filter
...
*/
void BallDetector::filterBlackSpots(Spot currentSpot,
                                    ...
                                    std::vector<Spot> & actualBlobs)
{
    ...
    if (whites < half_perimeter) {
        blobs.push_back(std::make_pair(currentSpot.x, currentSpot.y));
        actualBlobs.push_back(currentSpot);
        if (debugBall) {
                            ...
                            EdgeList& edges)
{
    ...
    for (auto i = spotter.begin(); i != spotter.end(); i++) {
        ...
        if (!(isWhite() || isGreen()) &&
            ...
            filterBlackSpots((*i), blackBlobs, actualBlackBlobs);
