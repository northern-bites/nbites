...
// Further filters the dark spots found by the spot filter
...
{
    // iterate through all dark spots
    for (auto i = darkSpots.begin(); i != darkSpots.end(); i++) {

        // convert dark spot's image coordinates back to raw coordinates
        int midX = (*i).ix() + width / 2;
        int midY = -(*i).iy() + height / 2;

                // make sure the midpoint of the dark spot is not too close to any
                // of the edges of the image? (what is the 5?)
		if (midX < width - 5 && midX > 5 && midY > 5 && midY < height - 5) {

                        // update the raw coordinates of the dark spot
			(*i).rawX = midX;
			(*i).rawY = midY;
                
                        // get the color of the midpoint of the spot?
			getColor(midX, midY);
			...
			if (!(isWhite() || isGreen()) &&
				(!topCamera || midY > field->horizonAt(midX))) {
                                ...
                                // check what filterBlackSpots does
				if (filterBlackSpots((*i))) {
					...
					actualBlackSpots.push_back((*i));
                                        ...
                                        // this spot is in fact a potential dark spot
...
// further filters the white spots found by the spot filter
    ...
    for (auto i = whiteSpots.begin(); i != whiteSpots.end(); i++) {
        ...
        if (filterWhiteSpot((*i), blackSpots, badBlackSpots)) {
            actualWhiteSpots.push_back((*i));
                std::cout << "--------------------------ACTUAL WHITE SPOTS-----------------------------" << std::endl;
                if(debugBall) {
...
/* 
   ...
   spot could reasonably be interpreted as one of the black
 ...
 * 
 * Inputs:
 * 	- x, y: the x- and y-coordinates of the center of the ball
 *      - radius: the radius of the ball
 * Outputs:
 *      - a boolean indicating whether the sanity check has been
 *        passed; [elaborate]
 *
 * Description: [figure out what this does]
 *
 ********************************************************************/
...
(intPairVector & blackSpots, spotVector & actualBlobs,
 double cameraHeight, bool & foundBall) {
    ...
    // an array that indicates the number of correlations for the
    // ith black blob
    int correlations[blackSpots.size()];

    // an array that indicates correlated blob pairs
    int correlatedTo[blackSpots.size()][blackSpots.size()];

    // indicates whether three black blobs have been found to be coordinated
    bool foundThree = false;

    // loop through filtered black blobs and see if any are close together
    for (int i = 0; i < blackSpots.size()-1; i++) {

        // get the current black spot
        ...
        for (int j = i+1; j < blackSpots.size(); j++) {
            ...
            // get current 2nd black spot
            std::pair<int,int> q = blackSpots[j];

            // if the current pair of blobs are close together...
            if (blobsAreClose(p, q)) {

                // update the appropriate inidices in the correlations arrays
                correlations[i] += 1;
                correlatedTo[i][j] = 1;
                ...
                if (correlations[i] > 2) { 
                    ...
                    // sum the x- and y-coordinates of the correlated blobs
...
std::pair<double,double> BallDetector::topOfBallBrighterThanBottomMedian(Spot spot) {
    ...
    int topTotalBrightness = 0, bottomTotalBrightness = 0;
    ...
    for (int y=topY; y<midY; y++) {
      for (int x=leftX; x<=rightX; x++, topPixels++) {
        ...
        topTotalBrightness += *(yImage.pixelAddr(x,y));  
    ...
    for (int y=midY + 1; y<=bottomY; y++) {
      for (int x=leftX; x<=rightX; x++, bottomPixels++) {
        ...
        bottomTotalBrightness += *(yImage.pixelAddr(x,y));  
      }
    }
    ...
    std::cout << "Top Half Mean brightness: " << topTotalBrightness/(topYvalues.size()*4) << std::endl;
    std::cout << "Bottom Half Mean brightness: " << bottomTotalBrightness/(bottomYvalues.size()*4) << std::endl;
    std::cout << "Top Half Median brightness: " << topMedian << std::endl;
    std::cout << "Bottom Half Median brightness: " << bottomMedian << std::endl;
								   ...
								   intPairVector & badBlack)
{
    ...
    if(debugBall) {
        // std::cout << "<WILL MARCUS FUNCTIONS>" << std::endl;

        // std::cout << "-------------------NOT ACTUAL WHITE SPOTS---------------"<<std::endl;
        // topOfBallBrighterThanBottomMean(spot);
        // topOfBallBrighterThanBottomMedian(spot);
        // aboveBallRectangleBrighterThanBelowBallRectangle(spot);
        // std::cout<<"filterWhiteSpot returned true\n";
        // debugDraw.drawPoint((*i).ix() + width / 2, -(*i).iy() + height / 2, RED);
    }
