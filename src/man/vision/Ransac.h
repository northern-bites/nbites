...
namespace RANSAC
{
   ...
   template <typename T> class Acceptor
   {
      public:
         ...
         bool accept(T &item, PointI p) const;
   ...
   template <> class Acceptor<RANSACCorner>
   {
      public:
         ...
         bool accept  (PointI p, float error);
         void finalise();
      private:
   ...
   */
    ...
    **/
   bool findLine(const std::vector<PointI>  &points,
    ...
    **/
   bool findLineConstrained(const std::vector<PointI>  &points,
                 std::vector<bool>        **cons,
                 RANSACLine                &result,
                 unsigned int               k,
                 float                      e,
    ...
    * @param            seed A seed value
    **/
   bool findCircleOfRadius(const std::vector<PointI>  &points,
                           float                      radius,
                           std::vector<bool>        **cons,
                           RANSACCircle              &result,
                           unsigned int               k,
       ...
       * @param            seed A seed value
       **/
      bool findLinesAndCircles(const std::vector<PointI>  &points,
                                     float                      radius,
                                     std::vector<bool>        **cons,
    ...
    * @param            seed A seed value
    **/
   bool findCircleOfRadius3P(const std::vector<PointI>  &points,
                             float                      radius,
                             float                      radius_e,
                             std::vector<bool>        **cons,
                             RANSACCircle              &result,
