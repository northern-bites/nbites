...
struct RANSACLine
{
   ...
   RANSACLine(PointI p1, PointI p2, float var = 0) : p1(p1), p2(p2), var(var)
   {
      t1 = p2.y() - p1.y();
      t2 = p1.x() - p2.x();
...
struct RANSACCircle
{
   ...
   RANSACCircle(const PointI p1, const PointI p2, const PointI p3, float var = 0)
      : var(var)
   {
   ...
   RANSACCircle(const PointI p1, const PointI p2, float radius, float var = 0)
