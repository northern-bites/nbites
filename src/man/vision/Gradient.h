...
public:
    ...
    static inline uint8_t dir(int y, int x) {
        return static_cast<uint8_t>(
            static_cast<int32_t>(atan2(y, x) / M_PI * 128.0) % 256);
    }
...
// **********************************************
    ...
    inline uint16_t getMagnitude(int i, int j) const {
#ifdef USE_MMX
    ...
    inline int16_t getX(int i, int j) const {
#ifdef USE_MMX
    ...
    inline int16_t getY(int i, int j) const {
#ifdef USE_MMX
