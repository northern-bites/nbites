        ...
        */
        for (int i = 0; i < 320; i++)
        {
            for (int j = 0; j < 240; j++)
            {
                ...
                do {
                    display = true;
                    float s = (float)r.getS(i, j) / 256.0f;
                    float h = (float)r.getH(i, j) / 256.0f;
                    float z = (float)r.getZ(i, j) / 256.0f;
                    int y = r.getY(i, j);
                    // Since H is an angle the math is modulo.
...
{
    float s = (float)r.getS(x, y) / 256.0f;
    float h = (float)r.getH(x, y) / 256.0f;
    float z = (float)r.getZ(x, y) / 256.0f;
    int yy = r.getY(x, y);
    int u = r.getU(x, y);
    int v = r.getV(x, y);
