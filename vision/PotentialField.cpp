...
inline static const float point_yGrad(const float attr, const float spr2, const float x_p,
		  const float y_p, const float x, const float y) {
...
inline static const float lineHeight(const float attr, const float spr2,
				     ...
				     const float c_l, const float x,
				     const float y) {
  return attr * pow((double)M_E, (double)(-dLine2(a_l, b_l, c_l, x, y) / spr2));
}
...
float dotProduct(const Vector2D a, const Vector2D b) {
  // Returns the dot product of two 2-dimensional vectors, represented as points
