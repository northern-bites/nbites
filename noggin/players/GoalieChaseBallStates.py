...
def goalieChase(player):
    if player.brain.ball.on and constants.USE_LOC_CHASE:
        return player.goNow('goaliePositionOnBall')
    elif transitions.shouldScanFindBall(player):
        return player.goNow('goalieScanFindBall')
    elif transitions.shouldApproachBall(player):
        return player.goNow('goalieApproachBall')
    elif transitions.shouldTurnToBall_ApproachBall(player):
        return player.goNow('goalieTurnToBallFar')
    elif transitions.shouldSpinFindBall(player):
        return player.goNow('goalieSpinFindBall')
    elif helper.shouldStopChase():
...
def goalieScanFindBall(player):
    ...
    if helper.shouldStopChase():
        return player.goLater('goaliePosition')
    elif player.brain.ball.on and constants.USE_LOC_CHASE:
        player.brain.tracker.trackBall()
        return player.goLater('goaliePositionOnBall')
    elif transitions.shouldTurnToBall_FoundBall(player):
        return player.goLater('goalieTurnToBallFar')
    elif transitions.shouldSpinFindBall(player):
...
def goalieTurnToBallFar(player):
    ...
    if helper.shouldStopChase():
        ...
        return player.goLater('goaliePositionForKick')
    elif transitions.shouldApproachBall(player):
        return player.goLater('goalieApproachBall')
    elif transitions.shouldScanFindBall(player):
...
def goaliePositionForKick(player):
    if player.firstFrame():
        player.stopWalking()
        player.currentChaseWalkX = 0
        player.currentChaseWalkY = 0
...
#                   " and " + str(constants.BALL_KICK_LEFT_X_FAR), "cyan")
    ...
    if transitions.shouldApproachForKick(player):
        sX = ball.locRelX
        player.printf("Ball print x is: " + str(), "cyan")
    else:
        sX = 0
    if helper.shouldStopChase():
        return player.goLater('goaliePosition')
    elif transitions.shouldKick(player):
        return player.goLater('goalieWaitBeforeKick')
    elif transitions.shouldScanFindBall(player):
        return player.goLater('goalieScanFindBall')
    elif ball.on:
...
def goalieWaitBeforeKick(player):
    if player.firstFrame():
        player.stopWalking()
        player.currentChaseWalkX = 0
        player.currentChaseWalkY = 0
...
#         return player.goLater('goaliePositionForKick')
        ...
        return player.goLater('goalieDecideKick')

def goalieDecideKick(player):
    """
    ...
    if player.firstFrame():
        ...
        player.brain.tracker.switchTo('goalieStopped')
    ...
    if (player.stateTime < SweetMoves.getMoveTime(SweetMoves.KICK_SCAN)):
        ...
        if constants.SUPER_SAFE_KICKS:
            return player.goLater('goalieIgnoreOwnGoal')
...
#                 return player.goLater('goalieKickBallRight')
#             else:
#                 return player.goLater('goalieKickBallLeft')
            # Goal in front
            ...
            if oppLeftPostBearing < 0 and oppRightPostBearing > 0:
                # kick straight
                return player.goLater('goalieKickBallStraight')
            else:
...
def goalieKickBallStraight(player):
    if player.firstFrame():
        player.brain.tracker.trackBall()
...
def goalieKickBallLeft(player):
    if player.firstFrame():
        ...
        return player.goLater('goalieKickBallLeftExecute')
    return player.stay()

def goalieKickBallLeftExecute(player):
    if player.firstFrame():
        player.brain.tracker.trackBall()
...
def goalieKickBallRight(player):
    if player.firstFrame():
        player.brain.tracker.trackBall()
...
def goalieIgnoreOwnGoal(player):
