...
public class WorldControllerPainter implements DogListener {
    ...
    public void drawBallUncertainty(Graphics2D drawing_on, 
				    ...
				    double x_uncert, 
				    double y_uncert) {
        field.drawOval(drawing_on, ESTIMATED_BALL_UNCERT_COLOR, field.DRAW_STROKE, 
    ...
    public void reportUpdatedBallLocalization(DogHistory dog_history, 
					      ...
					      int team_color, 
					      int player_number) {
        if (dog_history.estimated_ball_position_history.size() >= num_positions_to_draw) {
            dog_history.estimated_ball_position_history.removeLast();
        }
        dog_history.estimated_ball_position_history.addFirst(new_ball_localization_info);
	dog_history.color = team_color;
	dog_history.number = player_number;
    }
    ...
    public void reportTrackedLandmark(DogHistory dog_history, LocalizationPacket landmark_location) {
	...
	LocalizationPacket est = singleDogHistory().estimated_position_history.getFirst();
	double new_angle = AngleUtilities.sub180Angle(-angle+est.getHeadingEst());	
	
	double adj = Math.cos(new_angle * DEG_TO_RAD)*dist;
	double opp = Math.sin(new_angle * DEG_TO_RAD)*dist;
	//System.out.println("angle: " + angle + " dist: " + dist + " y_est: " + est.getYEst() + " x_est: " + est.getXEst() + " adj: " + adj + " opp: " + opp);

	by_x = opp+est.getXEst();
	by_y = adj+est.getYEst();
	//System.out.println("dist: " + dist + " angle: " + angle + " x: " + est.getXEst() + " y: " + est.getYEst() + " h: " + est.getHeadingEst());
    }
	...
	// Use the dog's unique number to see if we allready have it in our hash of all dogs, adding it to the hash if we don't
    ...
    public void transformFieldCoordinatesToRedTeamSide(LocalizationPacket loc_pack) {
	// Currently transforms x_ and y_est, heading_est, x_velocity, and y_velocity
	loc_pack.x_est = field.FIELD_GREEN_WIDTH - loc_pack.x_est;
	loc_pack.y_est = field.FIELD_GREEN_HEIGHT - loc_pack.y_est;
	loc_pack.heading_est = AngleUtilities.sub180Angle(loc_pack.heading_est + 180);
	loc_pack.x_velocity = loc_pack.x_velocity * -1.0;
	loc_pack.y_velocity = loc_pack.y_velocity * -1.0;
    }

    public void prepareForUDP() {
	// Get ready for the UDP communications, clear everything out, inititiailize arrays, etc
	// As of now, all of this gets taken care elsewhere
    }
