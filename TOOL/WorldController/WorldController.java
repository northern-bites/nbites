...
public class WorldController extends JPanel implements KeyListener,
    ...
    public static final int DOG_FPS = 30;
    public static final int DRAWING_FPS = DOG_FPS;
	...
	udp_server.addDogListener(painter);
        ...
        setSize((int) (the_field.getPreferredSize().getWidth() + BUTTON_AREA_WIDTH), (int) (the_field.getPreferredSize().getHeight()));
	...
	if (mode == VIEW_DOG_EKF || mode == SIMULATE_WITH_DOG_DATA
	    ...
	    aibo.sendCommand("removePythonControl");
	    aibo.sendCommand("motion " + x + " " + y + " " + spin);
	    key_pressed = true;
	}
        ...
        /*
          ...
          switch(key) {
              case KeyEvent.VK_EQUALS:
                  if (free_speed < MAX_SPEED_VARIABLE) {
                      free_speed++;
                  }
                  System.out.println("free_speed: " + free_speed);
                  break;
              case KeyEvent.VK_MINUS:
                  if (free_speed > MIN_SPEED_VARIABLE) {
                      free_speed--;
                  }
                  System.out.println("free_speed: " + free_speed);
                  break;
              case KeyEvent.VK_W:
                  free_forward += free_speed;
                  break;
              case KeyEvent.VK_S:
                  free_forward -= free_speed;
                  break;
              case KeyEvent.VK_A:
                  free_left += free_speed;
                  break;
              case KeyEvent.VK_D:
                  free_left -= free_speed;
                  break;
              case KeyEvent.VK_R:
                  free_rotate -= free_speed;
                  break;
              case KeyEvent.VK_F:
                  free_rotate += free_speed;
                  break;
          ...
          // converts motion variable speed to cm/frame
          if (free_forward > 0) {
              free_forward = ((MAX_SPEED_FORWARD / 13.) * free_forward / 15);
          }
          else if (free_forward < 0) {
              free_forward = ((MAX_SPEED_BACKWARD / 13.) * free_forward / 15);
          }
          if (free_left != 0) {
              free_left = ((MAX_SPEED_LATERAL / 13.) * free_left / 15);
          }
          if (free_rotate != 0) {
              free_rotate = ((MAX_SPEED_ROTATE / 13.) * free_rotate / 15);
          }
      */
        /*
      // prints out cm/s movement
      if (free_forward != 0) {
      System.out.println("free_forward: " + free_forward);
      }
      if (free_left != 0) {
      System.out.println("free_left: " + free_left);
      }
      if (free_rotate != 0) {
      System.out.println("free_rotate: " + free_rotate);
      }*/
    }
    ...
    public void stateChanged(ChangeEvent e) {
        ...
        if (!source.getValueIsAdjusting() && source == num_display_estimates_slide) {
            int value = (int)source.getValue();
